...
related_google_tbl <- read_rds(glue::glue("data/social/related_google_tbl_20220305.rds"))
...
draw_related_keywords_ysy <- function(hubo_name =  "윤석열") {
  ...
  network_viz_tbl %>%
    ...
    mutate(eigen = centrality_eigen(),
           group = group_infomap()) %>%
    mutate(관심사항 = ifelse(str_detect(name, "윤석열 안철수"), 1, 0 ) ) %>%   
    ggraph(layout='nicely') +
      geom_edge_link(color='gray50', alpha=.2) +
      geom_node_point(aes(color=factor(group), size=eigen)) +
      geom_node_text(aes(label=name), size=3, repel=TRUE) +
...
draw_related_keywords_acs <- function(hubo_name =  "안철수") {
  ...
  network_viz_tbl %>%
    ...
    mutate(eigen = centrality_eigen(),
           group = group_infomap()) %>%
    mutate(관심사항 = ifelse(str_detect(name, "단일화"), 1, 0 ) ) %>%   
    ggraph(layout='nicely') +
      geom_edge_link(color='gray50', alpha=.2) +
      geom_node_point(aes(color=factor(group), size=eigen)) +
      geom_node_text(aes(label=name), size=3, repel=TRUE) +
