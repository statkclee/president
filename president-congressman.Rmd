...
congressman_info_list <- map(congressman$의원명, people_info_possibly)

congressman_info_list %>% 
  write_rds("data/congressman_info_list.rds")

congressman_tbl <- map_df(congressman_info_list, rbind) %>% 
  mutate(의원명 = congressman$의원명) %>% 
  select(의원명, everything())

congressman_tbl %>% 
  write_rds("data/congressman_tbl.rds")
...
congressman_raw <- 
  read_rds("data/congressman_tbl.rds")

congressman_tbl <- congressman_raw %>% 
  select(의원명, 소속, sns) %>% 
...
- 유튜브: https://www.yuichiotsuka.com/youtube-data-extract-r/
...
# SNS_stat_gt %>% 
#   gtsave(filename = "SNS_stat_gt.png", path = "fig")
```
